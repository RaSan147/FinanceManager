class Goal {
    ...
    render() {
        ...
        if (this.hasAiData(g)) {
            ...
            const accordion = createEl('div', { class: 'acc-accordion mt-3' });
            const trigger = createEl('button', {
                type: 'button',
                class: 'acc-trigger',
                'aria-expanded': 'false'
            }, [
                createEl('span', { class: 'acc-trigger-chevron', 'aria-hidden': 'true' }),
                createEl('span', { class: 'acc-trigger-label' }, 'AI Plan')
            ]);
            const planBody = createEl('div', { class: 'acc-panel', hidden: '' });
            const scroller = createEl('div', { class: 'acc-content' });
            ...
            trigger.addEventListener('click', ()=>{
                ...
                item.querySelectorAll('.acc-accordion .acc-trigger[aria-expanded="true"]').forEach(btn=>{
                    if(btn!==trigger){
    ...
    static initGoalsPage() {
        ...
        if (addForm) {
            addForm.addEventListener('submit', (e) => {
                ...
                const id = fd.get('_id');
                if (!id) return; // creation handled elsewhere
                e.preventDefault();
                App.utils.withSingleFlight(addForm, async () => {
                    try {
                        const patchPayload = {
                        ...
                        await App.utils.fetchJSONUnified(`/api/goals/${id}`, {
                            method: 'PATCH',
