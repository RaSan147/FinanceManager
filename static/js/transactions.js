...
(function() {
  ...
  let modalEl, formEl, titleEl, submitBtn, idInput, symbolPrefixEl, currencySelect, categorySelect, typeSelect, dataList;
  let bsModal;
  ...
  function filterCategoriesForType(typeVal) {
    const incomeGroup = categorySelect.querySelector('[data-group-income]');
    const expenseGroup = categorySelect.querySelector('[data-group-expense]');
    if (!incomeGroup || !expenseGroup) return;
    if (typeVal === 'income') {
      incomeGroup.style.display = ''; expenseGroup.style.display = 'none';
      if (!incomeGroup.contains(categorySelect.selectedOptions[0])) categorySelect.value = '';
    } else {
      expenseGroup.style.display = ''; incomeGroup.style.display = 'none';
      if (!expenseGroup.contains(categorySelect.selectedOptions[0])) categorySelect.value = '';
    }
  }
